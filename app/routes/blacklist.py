"""Blacklist management (Section 31)."""
...
@login_required
def list_entries():
    entries = BlacklistEntry.query.filter_by(organization_id=current_user.organization_id).order_by(
        BlacklistEntry.created_at.desc()).all()
    return render_template("lists/blacklist.html", entries=entries)
...
def add():
    ...
    if not value:
        ...
        return redirect(url_for("blacklist.list_entries"))
    entry = BlacklistEntry(organization_id=current_user.organization_id, user_id=current_user.id,
                            entry_type=entry_type, value=value, notes=request.form.get("notes", ""))
    db.session.add(entry)
    db.session.commit()
    audit_service.log_event("blacklist_changed", user_id=current_user.id, target_type="blacklist",
...
@login_required
def export():
    entries = BlacklistEntry.query.filter_by(organization_id=current_user.organization_id).all()
    buffer = io.StringIO()
...
def import_csv():
    ...
    for row in reader:
        ...
        if not value:
            continue
        if BlacklistEntry.query.filter_by(organization_id=current_user.organization_id, value=value).first():
            continue
        ...
        count += 1
    db.session.commit()
    flash(f"Imported {count} blacklist entries.", "success")
    return redirect(url_for("blacklist.list_entries"))
